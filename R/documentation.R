...
#' \item isOptionEqualToValue `func` \cr Default is - Used to determine if the option represents the given value. Uses strict equality by default.  Both arguments need to be handled, an option can only match with one value.Signature:function(option: Value, value: Value) = booleanoption The option to test.value The value to test against.
...
#' \item loadingText `node` \cr Default is 'Loading80' Text to display when in a loading state.For localization purposes, you can use the provided translations.
...
#' \item disableRipple `bool` \cr Default is FALSE If true, the ripple effect is disabled.  Without a ripple there is no styling for :focus-visible by default. Be sure to highlight the element by applying separate styles with the .Mui-focusVisible class.
...
#' \item children `node` \cr Default is - The option elements to populate the select with. Can be some MenuItem when native is false and option when native is true. The MenuItem elements must be direct descendants when native is false.
#' \item classes `object` \cr Default is Override or extend the styles applied to the component.See CSS classes API below for more details.
...
#' \item defaultValue `any` \cr Default is - The default value. Use when the component is not controlled.
#' \item displayEmpty `bool` \cr Default is FALSE If true, a value is displayed even if no items are selected.In order to display a meaningful value, a function can be passed to the renderValue prop which returns the value to be displayed when no items are selected.  When using this prop, make sure the label doesn't overlap with the empty displayed value. The label should either be hidden or forced to a shrunk state.
#' \item IconComponent `elementType` \cr Default is ArrowDropDownIcon The icon that displays the arrow.
...
#' \item labelDisplayedRows `func` \cr Default is function defaultLabelDisplayedRows( from, to, count ) return `$from8093$to of $count !== -1 ? count : `more than $to``; Customize the displayed rows label. Invoked with a from, to, count, page object.For localization purposes, you can use the provided translations.
